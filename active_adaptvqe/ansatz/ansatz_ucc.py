...
class Fermionic_ex_UCC:
    ...
    def __init__(
        self,
        id_spin_up_occ: np.ndarray,
        id_spin_down_occ: np.ndarray,
        id_spin_up_vir: np.ndarray,
        id_spin_down_vir: np.ndarray,
        ...
        :param id_spin_up_occ: A numpy array of orbital indices for occupied up-spin electrons.
        :param id_spin_down_occ: A numpy array of orbital indices for occupied down-spin electrons.
        :param id_spin_up_vir: A numpy array of orbital indices for virtual up-spin electrons.
        :param id_spin_down_vir: A numpy array of orbital indices for virtual down-spin electrons.
        ...
        """
        self._idarr_spinup_occ = id_spin_up_occ
        self._idarr_spindown_occ = id_spin_down_occ
        self._idarr_spinup_vir = id_spin_up_vir
        self._idarr_spindown_vir = id_spin_down_vir
        self._chrdict = chrdict

        self._idarr_all_occ = np.concatenate((self._idarr_spinup_occ, self._idarr_spindown_occ))
        self._idarr_all_vir = np.concatenate((self._idarr_spinup_vir, self._idarr_spindown_vir))

        self._idarr_all_occ.sort()
        self._idarr_all_vir.sort()

        # total number of occupied orbitals.
        self._N_spin_occ = len(self._idarr_all_occ)
        # total number of virtual orbitals.
        self._N_spin_vir = len(self._idarr_all_vir)
        # total number of orbitals.
        self._N_spin_tot = self._N_spin_occ + self._N_spin_vir
    ...
    def _pdt_irreps(chr_table: dict) -> dict:
        ...
        :raises NotImplementedError: If the provided system has degeneracy.
        ...
        for arr in valuelist:
            for ele in arr:
                if ele == 0 or ele > 1:
                    raise NotImplementedError("Degenerate cases not implemented yet")
    ...
    def _get_config_singles(self, N_occ_to_ex: int, N_vir_to_ex: int, spin_type: str = "conserved") -> np.ndarray:
        ...
        idarr_vir_to_ex_down_spin = [ele for ele in idarr_vir_to_ex_all_spin if ele in self._idarr_spindown_vir]

        # - sanity checks -#
        # -----------------#
        if N_occ_to_ex > self._N_spin_occ:
            raise ValueError(
                "Desired number of occupied orbitals is higher than occupied orbitals available"
            )

        if N_vir_to_ex > self._N_spin_vir:
        ...
        match spin_type:
            case "conserved":
                for i in range(N_occ_to_ex):
                    tmp = self._idarr_all_occ[-1 - i]
                    if tmp in self._idarr_spinup_occ:
                        arr = [ele for ele in idarr_vir_to_ex_up_spin]
                        ex = list(map(lambda x: [x, tmp], arr))
                        op_dagger_op.extend(ex)
                    elif tmp in self._idarr_spindown_occ:
    ...
    def _get_config_custom_singles(self, exarr: np.ndarray) -> np.ndarray:
        ...
        :raises ValueError: If any of the orbital indices (occupied or virtual) is incorrect
                            for the system.
        :param exarr: A 2D numpy array of user provided excitation list (for generating
        ...
        for ex in exarr:
            if ex[0] not in self._idarr_all_vir or ex[1] not in self._idarr_all_occ:
                raise ValueError("Either incorrect virtual/occupied orbital number or both!")
            elif ex[0] in self._idarr_spinup_vir and ex[1] in self._idarr_spinup_occ:
                op_dagger_op.append([ex[0], ex[1]])
            elif ex[0] in self._idarr_spindown_vir and ex[1] in self._idarr_spindown_occ:
                op_dagger_op.append([ex[0], ex[1]])
            else:
                continue
    ...
    def _get_config_doubles(self, N_occ_to_ex: int, N_vir_to_ex: int, spin_type: str = "conserved") -> np.ndarray:
        ...
        :raises ValueError: If number of electrons to excite is less than 2,
                            ...
                            electrons in the active space is greater than total number
        ...
        idarr_vir_to_ex_up_spin = [ele for ele in N_vir_to_ex_all_spin if ele in self._idarr_spinup_vir]

        # index array of virtual down-spin orbitals in the active space
        idarr_vir_to_ex_down_spin = [ele for ele in N_vir_to_ex_all_spin if ele in self._idarr_spindown_vir]

        # index array of occupied up-spin orbitals in the active space
        idarr_occ_to_ex_up_spin = [ele for ele in N_occ_to_ex_all_spin if ele in self._idarr_spinup_occ]

        # index array of occupied down-spin orbitals in the active space
        idarr_occ_to_ex_down_spin = [ele for ele in N_occ_to_ex_all_spin if ele in self._idarr_spindown_occ]
    ...
    def _restrict_orbital_symmetry(self, N_occ_to_ex: int, N_vir_to_ex: int, orb_sym: dict) -> tuple[list]:
        ...
        symdict = self._pdt_irreps(self._chrdict)

        sym_order_map = {ele: i for i, ele in enumerate(self._chrdict.keys())}
